...
(function() {
    ...
    (function() {
        ...
        var h = {
            has: function(m, n) {
                return new RegExp("(^|\\s)" + n + "(\\s|$)").test(O(m).className)
            },
            add: function(m, n) {
                if (!this.has(m, n)) {
            ...
            remove: function(m, n) {
                if (this.has(m, n)) {
                    O(m).className = O(m).className.replace(new RegExp("(^|\\s)" + n + "(\\s|$)", "g"), "")
                }
            }
        };
        var M = {
        ...
        var Y = function(p) {
                ...
                function q() {
                    ...
                    if (s.getDate() != u.getDate() || s.getYear() != u.getYear() || s.getMonth() != u.getMonth()) {
                        return " - " + T[u.getMonth()] + " " + u.getDate() + ", " + u.getFullYear()
                    } else {
                        return ""
                    }
                }
        ...
        X.txt.autoLink = function(n, m) {
            ...
            return X.txt.autoLinkUsernamesOrLists(X.txt.autoLinkUrlsCustom(X.txt.autoLinkHashtags(n, m), options_links), m)
        };
        TWTR.Widget.ify = {
            autoLink: function(m) {
                options = {
                ...
                if (m.needle.entities) {
                    ...
                    if (m.needle.entities.media) {
                        options.urlEntities = options.urlEntities.concat(m.needle.entities.media)
                    }
                }
                if (X && X.txt) {
                    return X.txt.autoLink(m.needle.text, options).replace(/([@＠]+)(<[^>]*>)/g, "$2$1")
                } else {
        ...
        W.prototype = {
            ...
            run: function() {
                var m = this;
                this.job(function() {
                    m.decayRate = m.decayFn() ? Math.max(1, m.decayRate / m.decayMultiplier) : m.decayRate * m.decayMultiplier;
        ...
        function S(n) {
            var m = '<div class="twtr-tweet-wrap">         <div class="twtr-avatar">           <div class="twtr-img"><a target="_blank" href="https://twitter.com/intent/user?screen_name=' + n.user + '"><img alt="' + n.user + ' profile" src="' + n.avatar + '"></a></div>         </div>         <div class="twtr-tweet-text">           <p>             <a target="_blank" href="https://twitter.com/intent/user?screen_name=' + n.user + '" class="twtr-user">' + n.user + "</a> " + n.tweet + '             <em>            <a target="_blank" class="twtr-timestamp" time="' + n.timestamp + '" href="https://twitter.com/' + n.user + "/status/" + n.id + '">' + n.created_at + '</a> &middot;            <a target="_blank" class="twtr-reply" href="https://twitter.com/intent/tweet?in_reply_to=' + n.id + '">reply</a> &middot;             <a target="_blank" class="twtr-rt" href="https://twitter.com/intent/retweet?tweet_id=' + n.id + '">retweet</a> &middot;             <a target="_blank" class="twtr-fav" href="https://twitter.com/intent/favorite?tweet_id=' + n.id + '">favorite</a>             </em>           </p>         </div>       </div>';
        ...
        TWTR.Widget.prototype = function() {
            ...
            var o = t + "api." + q + "/1/";
            var u = 25000;
            var v = t + "widgets.twimg.com/j/1/default.gif";
            return {
                init: function(x) {
                    var w = this;
                    ...
                    if (x.version >= 2 && !TWTR.Widget.hasLoadedStyleSheet) {
                        if (x.creator) {
                            ...
                            s.loadStyleSheet(t + "widgets.twimg.com/j/2/widget.css", this.widgetEl)
                        }
                    }
                    ...
                    this.intervalJob = new Q(this.interval, this._loop, function(y) {
                        w._normalizeTweet(y)
                    });
                    return this
                },
                ...
                _fullScreenResize: function() {
                    var w = L("twtr-timeline", "div", document.body, function(x) {
                ...
                setFooterText: function(w) {
                    this.footerText = (P.def(w) && P.string(w)) ? w : "Join the conversation";
                    if (this._rendered) {
                ...
                _setUrl: function() {
                    ...
                    if (this._isProfileWidget) {
                        ...
                        if (this._isFavsWidget) {
                            this.url = this._includeEntities(this._base + "&callback=" + this._cb + y() + "&clientsource=" + this.source)
                        } else {
                            if (this._isListWidget) {
                ...
                setTheme: function(AB, w) {
                    ...
                    var y = "#" + this.id + " .twtr-doc,                      #" + this.id + " .twtr-hd a,                      #" + this.id + " h3,                      #" + this.id + " h4 {            background-color: " + this.theme.shell.background + x + ";            color: " + this.theme.shell.color + x + ";          }          #" + this.id + " .twtr-tweet a {            color: " + this.theme.tweets.links + x + ";          }          #" + this.id + " .twtr-bd, #" + this.id + " .twtr-timeline i a,           #" + this.id + " .twtr-bd p {            color: " + this.theme.tweets.color + x + ";          }          #" + this.id + " .twtr-new-results,           #" + this.id + " .twtr-results-inner,           #" + this.id + " .twtr-timeline {            background: " + this.theme.tweets.background + x + ";          }";
                    if (k.ie) {
                        y += "#" + this.id + " .twtr-tweet { background: " + this.theme.tweets.background + x + "; }"
                    }
                    s.css(y);
                    return this
                },
                ...
                _getWidgetHtml: function() {
                    ...
                    function AA() {
                        if (y._isProfileWidget) {
                            return '<a target="_blank" href="https://twitter.com/" class="twtr-profile-img-anchor"><img alt="profile" class="twtr-profile-img" src="' + v + '"></a>                      <h3></h3>                      <h4></h4>'
                        } else {
                            if (y._isSearchWidget) {
                                return '<h3><a target="_blank" style="color:' + y.theme.shell.color + '" href="https://twitter.com/' + y._getWidgetPath() + '">' + y.title + '</a></h3>                      <h4><a target="_blank" style="color:' + y.theme.shell.color + '" href="https://twitter.com/' + y._getWidgetPath() + '">' + y.subject + "</a></h4>"
                            } else {
                ...
                _createTweet: function(w) {
                    ...
                    w.created_at = this._isRelativeTime ? R(w.created_at) : Y(w.created_at);
                    this.tweet = new S(w);
                ...
                _prePlay: function(x) {
                    ...
                    if (x.error) {
                        ...
                        if (x.results && x.results.length > 0) {
                            ...
                            if ((this._isProfileWidget || this._isFavsWidget || this._isListWidget) && P.array(x) && x.length) {
                                this.newResults = true;
                                if (!this._profileImage && this._isProfileWidget) {
                                    ...
                                    this.setCaption('<a target="_blank" href="https://twitter.com/intent/user?screen_name=' + w + '">' + w + "</a>")
                                }
                ...
                _play: function() {
                    ...
                    if (this._isRelativeTime && (this._behavior == "all" || this._behavior == "preloaded")) {
                        H(this.byClass("twtr-timestamp", "a", true), function(x) {
                    ...
                    if (!this._isLive || this._behavior == "all" || this._behavior == "preloaded") {
                        H(this.results, function(y) {
                            ...
                            if (w._isProfileWidget || w._isFavsWidget || w._isListWidget) {
                ...
                _insertNewResultsNumber: function() {
                    ...
                    if (this.runOnce && this._isSearchWidget) {
                        var z = this.response.total > this.rpp ? this.response.total : this.response.results.length;
                        var w = z > 1 ? "s" : "";
                        var y = (this.response.warning && this.response.warning.match(/adjusted since_id/)) ? "more than" : "";
                        var x = document.createElement("div");
                        h.add(x, "twtr-new-results");
                        x.innerHTML = '<div class="twtr-results-inner"> &nbsp; </div><div class="twtr-results-hr"> &nbsp; </div><span>' + y + " <strong>" + z + "</strong> new tweet" + w + "</span>";
